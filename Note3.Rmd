...
其中 a1 是該商品提供之服務所帶來的愉快程度，a2 是該商品操作簡便性，α 是個介於 0 與 1 之間的數字，m 是購買此商品所要付出的金額。  
...
```{r}
Apps_10 <- list(
  c(0.38,0.74,0.19), c(0.39,0.46,0.26), c(0.56,0.3,0.32), c(0.54,0.31,0.40), 
...
```
**練習二**：中文組距  
原始資料收回後常會依某些數值區間來分成不同組別，且描述手法均類似："未滿 200,000 元"、"200,000 ~ 299,999 元"、"300,000 元及以上"。  
...
1. 請寫一個`str_cut(x, cutpoints)`函數，它能萬用的將 x 轉成依中文組距描述的類別資料。  
    ...
    * cutpoints: class character, 如：c("未滿200,000元", "200,000～299,999元", "300,000元及以上")  
    ...
    Value: 回傳值為x依中文組距描述分類後的factor。</font>
```{r}
