...
**練習 5.3.5**：在**範例 5.2.2**中我們建立了機器人 askSilly_weather。  
...
<font color="#0b5395">**範例 5.3.9**：成績等級。  
...
<font color="#0b5395">回到**範例 5.3.9**：成績等級。  </font>
```{r}
grade <- 80
if(grade >=90){
  print("優")
...
cut(a, c(1,2,3))
```
<font color="#0b5395">**範例 5.3.11**：成績等級。  
...
letterGrade
```
**範例 5.3.12**：依照**範例 5.3.11**，建立`compute_letterGrade(myGrade)`函數, 可以依 myGrade 不同而回傳 letterGrade 且出現要求文字。  
```{r}
...
```
#### 累計型 output
<font color="#0b5395">**範例 5.4.7**：Summation  
$$\sum_{i=1}^{10} a_{i}$$ 
...
<font color="#0b5395">**範例 5.4.9**：批改學生作業</font>
```{r}
...
<font color="#0b5395">**範例 5.4.10**：總共要擲幾次硬幣，才能得到五次正面。</font>
```{r}
sum <- 0
count <- 0
while(sum<5){
...
<font color="#0b5395">回到**範例 5.4.8**：每次抽一個介於 1 到 100 的整數 (抽出放回) 重複抽10次，但每次必須是 3 的倍數才留下來。
```{r}
# for loop
