...
<font color="#0b5395">**範例5.3.10**：</font>  
$$ f(n)=\begin{cases}n/2 & \mbox{if }n\mbox{ is even} \\
-(n+1)/2 & \mbox{if }n\mbox{ is odd}\end{cases}$$
```{r}
n<- 540
...
miniFun()
```
    + 若最後一個被執行的指令會產生 print 在螢幕的效果，則可以用`{...}->`或`<-{...}`將這 print 的值存出來。  
```{r}
result <- 
  {
    x <- 2
...
print(result2)
```
    + 依照上一個特性，`function(...){....}`的 body 若最後一個「被執行」的指令是會印在螢幕的值，如該值要當回傳值可以不用`return(...)`就可以產生值回傳效果。  
```{r}
myFun <- function(x,y){
  return(x**y+15)
}
