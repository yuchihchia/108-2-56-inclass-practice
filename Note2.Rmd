...
<font color="#0b5395">回到**範例 4.4.1**：學生資料，誰 1998 年(含)以後出生？</font>  
```{r}
...
```
<font color="#0b5395">**範例 4.4.4**：刑事案件被害者人數，「30_39歲以上受害者」的人數為多少？</font>
```{r}
...
<font color="#0b5395">回到**範例 4.4.1**：學生資料，誰「喜歡美食」且「血型為 B 型」？</font>
```{r}
str(example[c("name","hobby","bloodType")])
example$name[pick_loveFood & pick_bloodB]
```
<font color="#0b5395">回到**範例 4.4.1**：學生資料，誰「1998 以後(含)出生」且「不住大台北」？</font>
```{r}
...
1. **固定字元/組**：單寫一個字組或字元，表示那個 strip 固定不動只有一個出象。  
  例如：`2`表示只有「2」一個出象，`櫻桃`只有「櫻桃」一個出象。  
...
### 複製次數 Quantifiers  
`(..|..|...)`或`[...]`只是一條 reel strip。  
有時我們同樣的 strip 想連放很多條，可以在`(..|..|...)`或`[...]`後面加上：  

* `{n}`(n = 1, 2, 3, ...)：放 n 條 strip。  
* `{n,m}`(n, m = 1, 2, 3, ...)：放 n 到 m 條strip。  
* `?`：放 0 或 1 條的 strip。  
* `*`：放 0 條以上的 strip。  
* `+`：放 1 條以上的 strip。  
...
```
第 5 個表演有 4 個場次，想知道它們的日期時間：  
```{r}
map(dance[[4]]$showInfo, ~ .x$time) -> list_showTimes_dance5
