...
levels(covid19$`縣市`)
```
將`covid19$縣市`依縣市目前個案數目由大排到小。（hint: 可使用`table()`來得到個案數統計, 它會自然產生named integer vector）
```{r}
number <- table(covid19$`縣市`)
number <- sort(number, decreasing=T)
covid19$`縣市` <- factor(covid19$`縣市`, levels=names(number))
levels(covid19$`縣市`)
```
