...
w %>%
  ...
  h(.) -> z
```
<font color="#0b5395">再回到**範例 5.1.1**。</font>  
```{r}
...
print(highestPerson)
```
<font color="#0b5395">**範例 5.1.2**：由 cities 創出每個元素對應的北部、東部、中部 region factor。</font>  
```{r}
...
<font color="#0b5395">**範例 5.1.4**：找出 dataList 中元素 class 為 numeric 裡那些大於 50 的數字。</font> 
```{r}
dataList <- list(
  num=c(2,97,22,15,7),
  chr=c("2","97","22","15","7")
...
{.[[1]][.[[1]] > 50]} -> bigNumber
```
<font color="#0b5395">再回到**範例 5.1.3**。</font> 
```{r}
population %>% 
  {names(sort(.))} %>%
  factor(cities, levels=.) -> cities
